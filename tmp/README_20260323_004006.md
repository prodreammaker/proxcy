...
Cloudflare Worker for VLESS over WebSocket with a new mini entrypoint for lightweight deployments:
...
- `worker-mini.js`: active mini worker entrypoint used by `wrangler.toml`
- `worker.js`: original production worker
- `workerui.js`: original lighter UI-focused variant
...
Edit these defaults in `worker-mini.js`, or provide Worker environment variables:

- `USERID`: your VLESS UUID
- `PROXYIP`: fallback proxy host used for loop prevention
- `CLEAN_IPS`: comma-separated fronting hosts/IPs for generated config links
